...
import { LayoutDashboard, Briefcase, Users, BarChart3, Settings, LogOut, ClipboardList } from 'lucide-react';
        ...
        <div className="flex h-screen bg-gray-50">
            ...
            <div className="w-64 bg-white border-r border-gray-100 flex flex-col">
                <div className="p-6">
                    ...
                    <p className="text-xs text-gray-400 mt-1 uppercase tracking-wider">Recruiter Portal</p>
                </div>

                <nav className="flex-1 px-4 space-y-1">
                    {menuItems.map((item) => {
                        const isActive = location.pathname === item.path || (item.path === '/recruiter/dashboard' && location.pathname === '/recruiter');
                        return (
                            <Link
                                key={item.label}
                                to={item.path}
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                ...
                <header className="h-16 bg-white border-b border-gray-100 flex items-center justify-between px-8">
                    <h2 className="text-lg font-semibold text-gray-800">
                        {menuItems.find(item => item.path === location.pathname)?.label || 'Recruiter Portal'}
                    </h2>
